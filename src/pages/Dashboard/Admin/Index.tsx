...
import { useFetchAnalytics } from "@/hooks/organization/useFetchAnalytic";
...
const AdminDashboardHome: React.FC = () => {
    ...
    const { data: analytics, isLoading, refetch } = useFetchAnalytics(filter);
    ...
    useEffect(() => {
        ...
        switch (filter) {
            case "today":
                ...
                break;
            case "last_week":
                start = startOfWeek(subDays(today, 7), { weekStartsOn: 1 }); // Mon
                end = endOfWeek(subDays(today, 7), { weekStartsOn: 1 }); // Sun
                break;
            case "last_month":
    ...
    const recentData = useMemo(() => {
        if (!analytics?.sessions?.over_time) return [];

        // Sort sessions by period (descending, most recent first)
        const sorted = [...analytics.sessions.over_time].sort(
        ...
        return sorted.slice(0, 5).map((item, idx) => {
            ...
            return {
                id: item.id || idx.toString(), // fallback if API doesn’t return id
                sessionName: verticalName,
                sessionType: capitalizeWords(item.session_type_display || "Unknown Type"),
                date: formatDate(item.period), // use period as date
                duration: formatTime(item.formatted_duration || 0), // fallback to 0 if not present
            };
        });
    }, [analytics?.sessions?.over_time]);

    // Assuming session_breakdown is already fetched
    ...
    const cardData = useMemo(() => {
        if (!analytics?.sessions?.by_vertical) return [];

        // total new sessions = sum of counts
        const totalNewSessions = analytics.sessions.by_vertical.reduce(
        ...
        const data = [
            {
                icon: getIcon("Total New Sessions", 0),
                title: "Total New Sessions",
                value: totalNewSessions.toString(),
                direction: "up",
            ...
            ...analytics.sessions.by_vertical.map((item: any, index: number) => {
                ...
                return {
                    icon: getIcon(key, index + 1),
                    title: key,
                    value: (item?.count ?? 0).toString(),
                    direction: "up", // static for now
                    percent: 0,
                };
            }),
        ];
    ...
    const lineChartData = useMemo(() => {
        ...
        analytics.sessions.over_time.forEach((item: { period: string; count: number; vertical?: string }) => {
            if (!item?.period) return;

            const date = new Date(item.period);
            const label = date.toLocaleDateString(undefined, { day: "numeric", month: "short" }); // "1 Aug"
            ...
            row[key] = (row[key] || 0) + (item.count || 0);
        });
    ...
    const lineChartColors = {
        "Pitch Practice": "#252A39", // dark gray
        Presentation: "#40B869", // green
        "Public Speaking": "#F5B546", // yellow
        "Media Training": "#1E90FF", // blue
        "General Manager": "#FF6B6B", // red
        General: "#9B59B6", // purple (fallback for "general")
    };

    const areaChartData = useMemo(() => {
        if (!analytics?.user_growth) return [];

        return analytics.user_growth.map((item: any) => {
            const date = new Date(item.period);
            ...
            return {
                month: label,
                currentWeek: item.count || 0,
                previousWeek: 0, // percentage logic can come later if you want comparisons
            };
        });
    }, [analytics]);
    ...
    const donutChartData = [
        { name: "ActiveUsers", value: Number(analytics?.user_status.active || 0), fill: "#252A39" },
        { name: "InactiveUsers", value: Number(analytics?.user_status.inactive || 0), fill: "#B9BCC8" },
    ];
    ...
    const totalUsers = analytics?.user_status?.total || 0;
    const latestGrowth = analytics?.user_growth?.[analytics.user_growth.length - 1];
    const todayNewUsers = Number(latestGrowth?.count || 0);

    const userPercentage = totalUsers ? parseFloat(((todayNewUsers / totalUsers) * 100).toFixed(1)) : 0;
            ...
            <div className="py-4 px-2 justify-between hidden md:flex">
                ...
                <div className="flex items-center">
                    <select
                        ...
                        className="me-4 px-3 py-2 rounded-[8px]"
                    >
                        ...
                        <div className="dash__card p-4 flex flex-col h-full justify-between rounded-[12px] relative overflow-hidden">
                            ...
                            <div className="flex items-center">
                                ...
                                {/*<p className="gunmetal">
                                    {card.direction === "up" ? "more than" : "less than"} {timeCompareValue()}
                                </p>*/}
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex flex-wrap lg:items-stretch">
                <div className="w-full lg:w-6/9 lg:pe-2 mb-4 mt-3 shad__table">
                    <div className="dash__card px-5 py-4 rounded-[8px]">
                        {analytics?.sessions && <RecentSessions data={recentData} />}
                    </div>
                </div>
                        ...
                        <div className="w-full h-auto donut__chart">
                            ...
                            <div className="donut__details p-3">
                                ...
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <div className="flex items-center me-2">
                                        <p className="gunmetal">{analytics?.user_status.active}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex items-center me-2">
                                        <p className="gunmetal">{analytics?.user_status.inactive}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
