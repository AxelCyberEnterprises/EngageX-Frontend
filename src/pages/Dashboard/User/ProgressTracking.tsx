...
const ProgressTracking: React.FC = () => {
  ...
  const { data: goalsAndAchievement, isLoading, error: fetchGoalsAndAchievement } = useGoalsAndAchievement();
  const { data: progressTracking, isLoading: progressTrackingLoading, error: progressTrackingEerror } = useProgressTracking();

  useEffect(() => {
    if (fetchGoalsAndAchievement) {
      toast.error('Failed to fetch goals and achievement: ' + (fetchGoalsAndAchievement.message || 'Unknown error'));
    }
  }, [])

  useEffect(() => {
    if (progressTrackingEerror) {
      toast.error('Failed to fetch goals and achievement: ' + (progressTrackingEerror.message || 'Unknown error'));
    }
  }, [])
  ...
  const achievementData: Achievement[] = [
    {
      ...
      level: getLevel(Math.round((goalsAndAchievement?.vocal_variety ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      score: Math.round((goalsAndAchievement?.overall_captured_impact ?? 0) / 10),
      level: getLevel(Math.round((goalsAndAchievement?.overall_captured_impact ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      level: getLevel(Math.round((goalsAndAchievement?.emotional_impact ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      level: getLevel(Math.round((goalsAndAchievement?.body_language ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      score: Math.round((goalsAndAchievement?.transformative_communication ?? 0) / 10),
      level: getLevel(Math.round((goalsAndAchievement?.transformative_communication ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      level: getLevel(Math.round((goalsAndAchievement?.audience_engagement ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      level: getLevel(Math.round((goalsAndAchievement?.structure_and_clarity ?? 0) / 10)),
      total: 10,
    ...
    },
    {
      ...
      score: Math.round((goalsAndAchievement?.language_and_word_choice ?? 0) / 10),
      level: getLevel(Math.round((goalsAndAchievement?.language_and_word_choice ?? 0) / 10)),
      total: 10,
  ...
  const performanceCardsData = [
    ...
    },
    {
      ...
      title: "Vocal Range",
      value: `${Math.round(progressTracking?.overview_card.vocal_variety || 0)}%`,
  ...
  const sessions: Session[] = progressTracking?.recent_session.map((session: any) => {
  ...
  const chartData = progressTracking?.graph_data.map((item: any) => {
            ...
            {isLoading ? (
              <section className='grid lg:grid-cols-3 grid-cols-1 md:gap-6 gap-4'>
                {Array(9).fill(0).map((_, index) => (
                  <Card key={index} className='gap-0 px-4 py-4 rounded-[12px] border border-[#E0E0E0] shadow-[0px_2px_8px_0px_#252A3914] flex flex-col justify-between'>
          ...
          <div>
            ...
            <section className='grid lg:grid-cols-2 grid-cols-1 gap-6 mt-10'>
              ...
              <div className="pl-6 pt-4 rounded-lg border border-gray-200 bg-white shadow-none">
                ...
                <RecentSessionsTable data={filteredSessions} hidePagination={true} loadingRecentSessions={progressTrackingLoading}/>
              </div>
            </section>
