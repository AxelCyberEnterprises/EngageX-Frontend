...
export type SortOption = "max-date" | "min-date" | "max-impact" | "min-impact" | "max-duration" | "min-duration";
...
export function useGetSequence() {
    return useQuery<any>({
        ...
        queryFn: async () => {
            const response = await apiGet<any>(`/sessions/sequences/`, "default");
            return response;
        },
    });
}
...
export function useCompareSequences(id?: string) {
    ...
    return useQuery<any>({
        ...
        queryFn: async () => {
            ...
            const response = await apiGet<any>(`/sessions/compare-sequences/${id}`, "default");
            return response;
        },
        enabled: isValidId,
    });
