import PitchBoardRoom from "@/assets/images/jpegs/practice-bg-dashboard.jpeg";
import BoardRoom2 from "@/assets/images/jpegs/practice-bg.jpeg";
import BoardRoom1 from "@/assets/images/pngs/boardroom-1.png";
import ConferenceRoom from "@/assets/images/pngs/conference-room.png";

export const publicSpeakingVEOptions = [{ name: "Conference Room", value: "conference_room", src: ConferenceRoom }];

export const pitchPracticeVEOptions = [{ name: "Boardroom", value: "board_room_1", src: PitchBoardRoom }];

export const presentationPracticeVEOptions = [
    { name: "Boardroom 1", value: "board_room_1", src: BoardRoom1 },
    { name: "Boardroom 2", value: "board_room_2", src: BoardRoom2 },
];
