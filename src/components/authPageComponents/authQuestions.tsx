...
const AuthQuestions: React.FC = () => {
  ...
  const [selectedOptionsPlan, setSelectedOptionsPlan] = useState<
    {
      contentId: number;
  ...
  const planQuestion = questions.find(
    (q) => q.question === "What do you plan on doing?"
  );
  const roleQuestion = questions.find(
    (q) => q.question === "What role are you?"
  );
  const handleInputContent = (
    e: React.ChangeEvent<HTMLInputElement>,
    questionType: "plan" | "role"
  ) => {
    const selectedContentId = Number(e.target.value);

    if (questionType === "plan") {
      const foundOption = planQuestion?.content?.find(
        (option) => option.contentId === selectedContentId
      );

      if (!foundOption) return;

      setSelectedOptionsPlan((prev) => {
        let updated;

        if (e.target.checked) {
          // Add selected option
          updated = [...prev, foundOption];
        } else {
          // Remove unselected option
          updated = prev.filter((opt) => opt.contentId !== selectedContentId);
        }

        console.log("selected option plan: ", selectedOptionsPlan);

        // Update display based on new first option (if any)
        const first = updated[0];

        setSelectedOptions((prevOptions) => ({
          ...prevOptions,
          planDisplay: first?.plan,
          purpose: first?.apiName as Purpose,
        }));

        dispatch(
          setSignupData({
            ...signupData,
            planQuestion: first?.plan || "",
          })
        );
    ...
    <div className="flex flex-col my-5 md:justify-center md:pt-12 scrollbar-hide overflow-y-auto scroll-smooth h-[100dvh] md:pb-10  lg:w-[90%] md:mx-auto max-lg:p-5">
          ...
          <div>
            ...
            <div className="mt-3 w-full flex flex-col md:flex-row gap-4 space-y-">
              {planQuestion.content.map((option) => {
                const isSelected = selectedOptionsPlan.some(
                  (opt) => opt.contentId === option.contentId
                );
            ...
            <div className="mt-3 w-full md:flex-row justify-center items-center mx-auto grid-cols-2 gap-2 flex-wrap flex flex-col space-y-4">
              {roleQuestion.content.map((option) => {
                ...
                return (
                  <label
                    key={option.contentId}
                    className={`flex items-center  md:max-w-[48%] ${
                      option.contentId === 7 && "md:max-w-max"
                    } text-sm px-3 py-3 w-full max-w-fi rounded-lg border cursor-pointer ${
                      isSelected
                        ? "bg-[#2154cb11] text-[#2154cb]"
                        : "text-black bg-white border-[#b7b7b7]"
                    }`}
      ...
      <button
        ...
        className={`w-full md:w-[80%]  ${
          !selectedOptions.purpose || !selectedOptions.user_intent
            ? "bg-[#3d3d3d62] hover:bg-[#3d3d3d62] cursor-not-allowed"
            : " hover:bg-accent-foreground  bg-primary"
        } mx-auto lg:w-full rounded-lg font-[montserrat] mt-7 py-4 text-white`}
      >
        Continue
      </button>
    </div>
